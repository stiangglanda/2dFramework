...
tile::tile(std::vector<frame> animatedtiles, SDL_Texture* tset, int x, int y, int tx, int ty, int w, int h,
           bool animated, bool Obstacle)
	: sheet(tset), x(x), y(y), tx(tx), ty(ty), width(w), height(h), mAnimatedtiles(animatedtiles), mAnimated(animated),
	  mObstacle(Obstacle)
{
}
			...
			for (auto y = 0; y < rows; ++y)
			{
				for (auto x = 0; x < cols; ++x)
				{
					...
					SDL_QueryTexture(tilesets[tset_gid],
					                 nullptr, nullptr, &ts_width, &ts_height);
